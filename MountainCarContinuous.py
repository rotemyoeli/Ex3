...
def PolicyNetwork(state, env_name):
    n_hid_1 = 40
    n_hid_2 = 40

    # action size (the output layer) = 3 is the maximum for all 3 env (cartpole = , mountaincar = , acrobot = )
    n_out = 3
    ...
    with tf.variable_scope("PolicyNetwork"):
        ...
        hidden1 = tf.layers.dense(state, n_hid_1, tf.nn.elu, init_xavier)
        hidden2 = tf.layers.dense(hidden1, n_hid_2, tf.nn.elu, init_xavier)
        mu = tf.layers.dense(hidden2, n_out, None, init_xavier)
        sigma = tf.layers.dense(hidden2, n_out, None, init_xavier)
        dum_output = tf.layers.dense(hidden2, n_out, None, init_xavier)
...
# function to normalize states used for mountaincar only
def scale_state(state):  # requires input shape=(2,)
    scaled = scaler.transform([state])
    while len(scaled) < 6:
        scaled = np.append(scaled, 0)
...
# In case of MountainCar used alone
#state_size = np.prod(env.observation_space.shape)

# state size (the input layer) =6 is the maximum for all 3 env (cartpole = , mountaincar = 2, acrobot = 6)
state_size = 6
...
action_tf_var, norm_dist = PolicyNetwork(state_placeholder, env_name)
